    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-black selection:bg-white selection:text-black">
        ...
        <GlassCard className="w-full max-w-2xl p-8 sm:p-14 space-y-10 relative z-10 border-white/[0.08] bg-black/40 text-center shadow-[0_30px_100px_rgba(0,0,0,0.8)] backdrop-blur-3xl">
          {/* Entrance Sequence Item 1: Logo Fades In */}
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.4, delay: 0.0, ease: "easeOut" }}
            className="mx-auto inline-flex w-20 h-20 rounded-[2rem] bg-gradient-to-br from-white/10 to-white/5 border border-white/10 items-center justify-center mb-4 shadow-[0_0_40px_rgba(255,255,255,0.05)] relative overflow-hidden group"
          >
          ...
          <div className="space-y-6">
            {/* Entrance Sequence Item 2: Heading Scales In */}
            <motion.div
            ...
            {/* Entrance Sequence Item 3: Subtitle Fades In */}
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4, delay: 0.4, ease: "easeOut" }}
              className="flex items-center justify-center gap-4 text-xs font-mono text-neutral-500 uppercase tracking-widest font-semibold"
            >
            ...
            {/* Entrance Sequence Item 4: Card Slides Upward */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.6, ease: "easeOut" }}
              className="max-w-md mx-auto space-y-4 text-neutral-400 text-sm sm:text-base leading-relaxed font-medium bg-white/5 p-6 rounded-2xl border border-white/5 shadow-[0_0_20px_rgba(255,255,255,0.02)]"
            >
              <p>&quot;I&apos;m your AI Founder Partner.</p>
              <p>
                I&apos;ll help you validate your startup, create your business strategy, build your roadmap, generate
                documents and guide you from idea to scale.&quot;
              </p>
            </motion.div>
          </div>

          {/* Entrance Sequence Item 5: Button Fades In & Lifts (Total sequence < 1.5s) */}
          <motion.div
          ...
          >
            <motion.div whileHover={{ scale: 1.03, y: -2 }} whileTap={{ scale: 0.97 }}>
              <GlowingButton
                ...
                size="lg"
                className="w-full sm:w-auto px-12 font-semibold shadow-[0_0_25px_rgba(255,255,255,0.2)]"
      ...
      <div className="w-full max-w-4xl flex flex-col rounded-[2.5rem] overflow-hidden border border-white/[0.08] bg-black/40 backdrop-blur-3xl shadow-[0_30px_100px_rgba(0,0,0,0.8)] h-[85vh] min-h-[600px] max-h-[900px] animate-in fade-in zoom-in-95 duration-700 z-10 relative">
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-white/[0.08] bg-white/[0.01]">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full bg-white/10 border border-white/10 text-[9px] font-mono font-bold text-white uppercase tracking-wider">
                  Beta
                </span>
              </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            {/* Go To Dashboard Secondary Glassmorphism Button (Requirement 5) */}
            <motion.button
              whileHover={{ scale: 1.03, y: -1 }}
              whileTap={{ scale: 0.97 }}
              onClick={handleGoToDashboard}
              className="group flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/15 text-white text-xs font-medium backdrop-blur-md transition-all shadow-sm focus:outline-none focus:ring-2 focus:ring-white/30 cursor-pointer"
              tabIndex={0}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-2xl px-6">
                {[
                  ...
                  <motion.button
                    ...
                    className="flex items-center gap-3 p-4 rounded-2xl bg-white/[0.03] border border-white/[0.08] hover:bg-white/[0.08] hover:border-white/20 transition-all text-left text-sm font-medium text-neutral-300 hover:text-white group shadow-lg cursor-pointer"
                  >
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-2xl flex items-center justify-center text-sm font-bold shrink-0 relative overflow-hidden shadow-lg ${
                      isUser
                        ? "bg-gradient-to-br from-neutral-700 to-neutral-900 border border-white/10 text-white"
                        : "bg-gradient-to-br from-white/10 to-white/5 border border-white/10 text-white shadow-[0_0_20px_rgba(255,255,255,0.05)]"
                    }`}
                  >
                  ...
                  <div className={`space-y-1.5 max-w-[85%] sm:max-w-[75%] ${isUser ? "text-right" : ""}`}>
                    <div
                      className={`flex items-center space-x-2 text-[10px] text-neutral-500 font-mono ${
                        isUser ? "justify-end" : ""
                      }`}
                    >
                    ...
                    <div
                      className={`p-5 rounded-2xl text-[15px] leading-relaxed shadow-lg transition-all ${
                        isUser
                          ? "bg-gradient-to-br from-neutral-200 to-white text-black font-medium rounded-tr-sm"
                          : "bg-white/[0.03] backdrop-blur-md border border-white/[0.08] text-neutral-200 rounded-tl-sm hover:border-white/15"
                      }`}
                    >
                      {isUser ? (
        ...
        <div className="p-6 bg-black/20 border-t border-white/[0.08] backdrop-blur-xl relative z-20">
          <form
            onSubmit={handleSend}
            className="relative flex items-end gap-3 p-2 bg-white/[0.03] border border-white/[0.12] rounded-[2rem] shadow-[0_10px_40px_rgba(0,0,0,0.5)] focus-within:bg-white/[0.05] focus-within:border-white/30 focus-within:shadow-[0_0_25px_rgba(255,255,255,0.08)] transition-all"
          >
            <button
              type="button"
              className="p-3 text-neutral-500 hover:text-white transition-colors shrink-0 rounded-full hover:bg-white/10 self-end cursor-pointer"
            >
            ...
            <div className="flex items-center gap-1 self-end pr-1 pb-1">
              <button
                type="button"
                className="p-2.5 text-neutral-500 hover:text-white transition-colors shrink-0 rounded-full hover:bg-white/10 cursor-pointer"
              >
