...
const prepColors = (input: any) => {
  ...
  for (let i = 0; i < MAX_COLORS; i++) {
    const col = base[Math.min(i, base.length - 1)];
    if (typeof col === 'string') {
      arr.push(hexToRGB(col));
...
export default function Iridescence({
  ...
  const activeColors = color ? (Array.isArray(color) && typeof color[0] === 'number' ? [color] : color) : colors;
  const activeMouse = mouseReact !== undefined ? mouseReact : mouseInteraction;
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uMouseEnabled: { value: activeMouse ? 1 : 0 },
    ...
    return () => {
      ...
      if (activeMouse) canvas.removeEventListener('pointermove', onPointerMove);
      ro.disconnect();
